...
const CourseCollegesComponent = () => {
  ...
  return (
    <div className="bg-gradient-to-b from-teal-50 to-indigo-50 min-h-screen">
      {/* Back button */}
      ...
      {selectedCourseDetails ? (
            ...
            <div className="flex gap-2">
              <div className="relative">
                <input
                  ...
                  className="w-full md:w-64 pl-10 pr-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                  value={searchTerm}
              ...
              <div className="flex flex-wrap gap-2 items-center">
                <button 
                  ...
                  className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                    modeFilter === 'all' ? 'bg-teal-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                  }`}
                >
                  All Modes
                </button>
                <button 
                  ...
                  className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                    modeFilter === 'online' ? 'bg-teal-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                  }`}
                >
                  Online
                </button>
                <button 
                  ...
                  className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                    modeFilter === 'distance' ? 'bg-teal-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                  }`}
                >
                  Distance
                </button>
                ...
                <div key={index} className="bg-white rounded-xl overflow-hidden shadow-lg flex flex-col md:flex-row">
                  <div className="md:w-1/3 lg:w-1/4 relative">
                    ...
                    <div className={`absolute top-4 left-4 px-3 py-1 rounded-md text-sm font-medium text-white ${
                      university.mode.toLowerCase().includes('online') ? 'bg-teal-600' : 'bg-indigo-600'
                    }`}>
                      {university.mode}
                    </div>
                  </div>
                  ...
                  <div className="p-6 md:w-2/3 lg:w-3/4">
                    <div className="flex flex-col md:flex-row justify-between mb-6">
                      <h3 className="text-xl font-bold text-teal-800 mb-2 md:mb-0">
                        {university.name}
                      </h3>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                      <div>
                        ...
                        <p className="flex items-center gap-1 text-gray-800 font-semibold">
                          <FaRupeeSign className="text-teal-600" /> {university.fee}
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="flex items-center gap-1 text-gray-800">
                          <FaCheck className="text-teal-600" /> {university.approvals}
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="flex items-center gap-1 text-gray-800">
                          <FaBriefcase className="text-teal-600" /> {university.advantage}
                        </p>
                      </div>
                    ...
                    <div className="flex gap-3">
                      <button className="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-md transition-colors flex items-center gap-2">
                        Get Help <FaArrowRight className="text-sm" />
                      </button>
                      <button className="px-4 py-2 bg-teal-100 hover:bg-teal-200 text-teal-800 rounded-md transition-colors">
                        Know More
                      </button>
                    </div>
                  </div>
        ...
        <div className="container mx-auto px-4 py-20 text-center">
          ...
          <button 
            onClick={goBack}
            className="px-6 py-3 bg-teal-600 hover:bg-teal-700 text-white rounded-lg transition-colors flex items-center gap-2 mx-auto"
          >
